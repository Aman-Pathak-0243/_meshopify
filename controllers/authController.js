...
export const updateProfileController = async (req, res) => {
  try {
    ...
    if (password && password.length < 6) {
      return res.json({ error: "Passsword is required and should have 6 characters" });
    }
...
export const getUsers = async (req, res) => {
  ...
  try {
    const usersCount = await userModel.countDocuments(); // Use the correct model (userModel)

    const users = await userModel
      .find()
      .skip((page - 1) * limit) // Skip based on page and limit
      .limit(parseInt(limit)) // Limit to the specified number of users
